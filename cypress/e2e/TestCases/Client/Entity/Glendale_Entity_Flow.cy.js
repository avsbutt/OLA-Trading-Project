...
import { DueDiligenceFormPage } from "@Pages/Client/Entity/DueDiligenceFormPage"
import{ OwnersAndOfficialsPage} from "@Pages/Client/Entity/OwnersAndOfficialsPage"
...
describe("ENTITY - Glendale(Margin Account) - Broker - Representative - Supervisor", () => {
  afterEach(() => {
    cy.clearCookies();
  ...
  it('Verify that US Citizen User can Create an Entity Account || Due Diligence Questionnaire Should not be filled || ID Type #Driver License', ()=>{
    ...
    TC_DisclosureSignaturesPage.AcknowledgeCheckbox()
...
describe("ENTITY - Glendale(Cash Account) - Broker - Representative - Supervisor", () => {
  afterEach(() => {
    cy.clearCookies();
    cy.clearLocalStorage();
  })
  ...
  it("Verify that Supervisor Can Download and Approved an Application", () => {
    LoginUtils('glendale', 'supervisor');
    waitForLoaderToDisappearUtils();
    CloseToasterIfAppearUtils();
